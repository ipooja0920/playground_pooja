...
i. Generate a random sample of size 𝑛 = 30 from a 𝑁 (100, 4) distribution starting from a\
...
\
(ii) Generate a random sample of size 𝑛 = 300 from a 𝑁 (100, 4) distribution starting from a random seed = 123457. Find the sample mean and variance.
...
**Question 3:** Suppose 𝐴 denotes an event that a statistics seminar ends on time and 𝐵 is the event that a sociology seminar ends on time. Suppose 𝐴 and 𝐵 are independent events, with 𝑃 (𝐴) = 0.85 and 𝑃 (𝐵) = 0.6.

We are told:

𝑃 (𝐴) = 0.85 (statistics seminar ends on time)

𝑃 (𝐵) = 0.6 (sociology seminar ends on time)

(𝐴) and (𝐵) are independent variables
...
Hence, P(A∩B)=**0.51** or there is a **51%** probability that both seminars will end on time.

b.  *What is the probability that neither seminars end on time.*

    -   Statistics seminar does **not** end on time → event A_Complement

    -   Sociology seminar does **not** end on time → event B_Complement

        P(A_Complement) =1−P(A) =1−0.85 = 0.15

    P(b_Complement) =1−P(A) =1−0.60 = 0.40

    Since A and B are independent variables, their complement also remains independent

    P(A_Complement ∩ B_Complement) = P(A_Complement) X P(B_Complement)

    P(A_Complement ∩ B_Complement) = 0.15×0.40=0.06
...
c.  *What is the probability that exactly one of them ends on time?*
    ...
    P ( exactly one) = P ( A ∩ B_Complement) + P ( A_Complement ∩ B )

    -   P ( A ∩ B_Complement ) = P(A) X P(B_Complement)

        =0.85×(1−0.6)=0.85×0.40=0.34

    -   P ( A_Complement ∩ B ) = P(A_Complement) X P(B)

        =(1−0.85)×0.60=0.15×0.60=0.09

    P(exactly one)=0.34+0.09=0.43

    Hence, the probability that exactly one seminar ends on time is **0.43 (43%)**.

d.  *Are the two events 𝐴 and 𝐵 mutually exclusive? Explain your answer.*

    Mutually exclusive events cannot occur together. Since P(A∩B)=0.51 which is not equal to 0, the events **can occur together** (both seminars ending on time).\
...
b.  The medical practitioners wish to statistically verify whether the true means of the\
    change in health status are the same or significantly different in the two groups. Set up and construct a suitable hypothesis test. Use the 𝑝-value to reach a decision at level of significance 𝛼 = 0.05
