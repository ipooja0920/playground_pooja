...
class TestPatternLibraryStructure:
    ...
    def test_exactly_25_patterns(self):
    ...
    def test_ids_are_sequential_1_to_25(self):
...
class TestSubPatterns:
    ...
    def test_high_priority_patterns_have_sub_patterns(self):
        """Graph Traversals, Backtracking, Dynamic Programming must all have sub_patterns."""
    ...
    def test_dp_has_2d_grid_sub_pattern(self):
        ...
        dp = next(p for p in PATTERNS if p["name"] == "Dynamic Programming (Knapsack, Range DP)")
    ...
    def test_greedy_has_two_pass_sub_pattern(self):
        """Greedy & Interval Partitioning must have a Two-Pass sub-pattern (the Candy fix)."""
        greedy = next(p for p in PATTERNS if p["name"] == "Greedy & Interval Partitioning")
        sub_names = [sp["name"] for sp in greedy.get("sub_patterns", [])]
        assert any("Two-Pass" in n or "Two Pass" in n for n in sub_names), (
            "Greedy & Interval Partitioning missing Two-Pass sub-pattern"
        )


class TestNotSignals:

    def test_graph_traversals_has_not_signal(self):
        gt = next(p for p in PATTERNS if p["name"] == "Graph Traversals (BFS, DFS)")
        signals = " ".join(gt["when_to_use"]).lower()
        assert "not" in signals

    def test_backtracking_has_not_signal_for_counting(self):
        bt = next(p for p in PATTERNS if p["name"] == "Backtracking & Recursive Search")
...
class TestValidPatternNames:
    ...
    def test_valid_names_set_has_25_entries(self):
        assert len(_VALID_PATTERN_NAMES) == 25
    ...
    def test_known_valid_pattern_accepted(self):
        assert "dynamic programming (knapsack, range dp)" in _VALID_PATTERN_NAMES

    def test_greedy_is_valid_pattern(self):
        assert "greedy & interval partitioning" in _VALID_PATTERN_NAMES
