...
Uses LlamaIndex Workflows to implement a 3-step async pipeline:
  Step 1: Query Synthesis — Translate natural language → SQL
  Step 2: Query Execution — Execute SQL against domain database
  Step 3: Answer Generation — Generate natural language response from results

Adapted from:
  - LlamaIndex workflows: https://docs.llamaindex.ai/en/stable/examples/workflow/advanced_text_to_sql/
  - Langfuse integration: https://langfuse.com/docs/integrations/llama-index/get-started/
"""
...
class TAGWorkflow(Workflow):
    ...
    def __init__(self, vec_db_manager, chat_db_manager, llm, *args, **kwargs):
        ...
        self.query_engine = NLSQLTableQueryEngine(
            ...
            llm=self.llm,
            sql_only=True,  # Only generate SQL queries, don't execute
        )
    ...
    def _is_valid_sql(self, sql: str) -> bool:
        ...
        return (
            ...
            or (sql.startswith('CREATE TABLE') and 'AS SELECT' in sql)
        )
    ...
    async def query_synthesis(
        ...
        try:
            ...
            print("Valid SQL:", valid_sql)

            if not valid_sql:
                return StopEvent(
                    result="I cannot generate a valid SQL query for this question. "
                    "The required information might not be available in the database."
                )

            # Extract relevant tables from the SQL query
            relevant_tables = [
                table for table in all_tables if table.lower() in sql_query.lower()
            ]
            ...
            return StopEvent(result=f"Error generating SQL query: {str(e)}")

    @step
    async def query_execution(
        self, ctx: Context, ev: QuerySynthesisEvent
    ) -> Union[QueryExecutionEvent, StopEvent]:
        """Step 2: Query Execution — Execute SQL against the database."""
        try:
            ...
            return StopEvent(result=f"Error executing SQL query: {str(e)}")
    ...
    async def answer_generation(
        ...
        try:
            ...
            result_str = (
                "\n".join([str(r) for r in ev.results])
                if ev.results
...
async def run_tag_pipeline(
    ...
    """Run the TAG pipeline with given parameters (standalone usage)."""
    ...
    if llm_provider == "OpenAI":
        ...
        llm = Anthropic(model="claude-3-5-sonnet-20241022", temperature=temperature)
...
async def main():
    """CLI entrypoint for running the TAG pipeline."""
    import argparse
